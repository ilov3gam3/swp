...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
            ...
            <div class="header-search"> <!-- Added a container for search form -->
                <form action="" method="post">
                    <c:if test="${not empty current_spe_id}">
                        <input type="hidden" value="${current_spe_id}" name="speciality" id="speciality">
                    </c:if>
                    <c:if test="${empty current_spe_id}">
                        <input type="hidden" value="0" id="speciality" name="speciality">
                    </c:if>
            ...
            <div class="specialty-list" >
                <div class="specialty-item" onclick="setSpeciality(0)">
                    <a class="h3 ${current_spe_id == 0 ? "text-warning" : "text-dark"} text-decoration-none mr-3 font-weight-bold" id="0">Tất cả</a>
                </div>
                <c:forEach items="${speciality_list}" var="item">
                    <div class="specialty-item" onclick="setSpeciality(${item.id})">
                        <a class="h3 ${current_spe_id == item.id ? "text-warning" : "text-dark"} text-decoration-none mr-3" id="${item.id}">
                            ${item.getName()}
                        </a>
                    </div>
                </c:forEach>
                            ...
                            <div class="doctor-card">
                                <div class="blog-img-cap">
                                    ...
                                    <div class="blog-cap">
                                        ...
                                        <div class="degree">${item.getSpeciality_name()}</div>
